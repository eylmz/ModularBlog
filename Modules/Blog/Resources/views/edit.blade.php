                ...
                <div class="panel-body">
                    <form method="post" action="{{ route('blogs.update', $post["id"]) }}">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" name="name" id="name" value="{{ old('name', $post["name"]) }}">
                        ...
                        <div class="form-group">
                            ...
                            <select multiple id="category" name="categories[]" class="form-control">,
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ in_array($category->id, old('categories', $categoryIDs))  ? 'selected':null }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select multiple id="tag" name="tags[]" class="form-control">
                                @foreach($tags as $tag)
                                    <option value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $tagIDs)) ? 'selected':null }}>{{ $tag->name }}</option>
                                @endforeach
                            </select>
                        </div>
